...
public class ClientFormationDetailsController {
    ...
    private void loadVideo(String filename,int progressValue){
        ...
        try{
            ...
            stopBtn.setOnAction(e -> {
                ...
                timeLabel.setText("00:00 / " + formatTime(player.getTotalDuration().toSeconds()));
            });
            player.setOnReady(() -> {
                ...
                timeLabel.setText(formatTime(0) + " / " + formatTime(totalSeconds));
                ...
                player.currentTimeProperty().addListener((obs, oldTime, newTime) -> {
                    ...
                    timeLabel.setText(
                            formatTime(newTime.toSeconds()) + " / " + formatTime(totalSeconds)
                    );
                });
    ...
    private void showQuizResult(int score, int total){
        ...
        double percent = (score * 100.0) / total;
        boolean success = percent >= 60;

        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Résultat du Quiz");
        alert.setHeaderText(success ? "🎉 Félicitations !" : "❌ Quiz non validé");

        String message =
                "Formation : " + formation.getTitre() + "\n\n" +
                        "Score : " + score + " / " + total + "\n" +
                        "Résultat : " + String.format("%.0f", percent) + " %\n\n" +
                        "Statut : " + (success ? "RÉUSSI ✔" : "ÉCHOUÉ");

        alert.setContentText(message);

        ButtonType revoirBtn = new ButtonType("Revoir formation");
        ButtonType retourBtn = new ButtonType("Retour catalogue");

        alert.getButtonTypes().setAll(revoirBtn, retourBtn);

        Optional<ButtonType> choice = alert.showAndWait();
        quizBox.setDisable(true);
        quizBox.setOpacity(0.6);
        if(choice.isPresent()){
            if(choice.get() == retourBtn){
                Router.goToFormationList();   // you already have Router
            }
            else{
